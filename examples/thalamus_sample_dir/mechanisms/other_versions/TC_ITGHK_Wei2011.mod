...
:
	m_inf = 1.0 / ( 1 + exp(-(v+shift+actshift+57)/6.2) ) :EI: from fig. 8 Wei 2011 should be the same as in Destexhe 98
	h_inf = 1.0 / ( 1 + exp((v+77)/4.0) )

	:tau_m = ( 0.612 + 1.0 / ( exp(-(v+shift+actshift+132)/16.7) + exp((v+shift+actshift+16.8)/18.2) ) ) / phi_m
	tau_m = 5*(0.204 + 0.333)/(exp(-(v+131)/16.7)+exp((v+15.8)/18.2))/phi_m
	if( (v) < -81) {
		tau_h = 0.333*exp((v+466)/66.6) / phi_h
	} else {
		tau_h = ( 9.32 + 0.333*exp(-(v+21)/10.5) ) / phi_h
	}
}
