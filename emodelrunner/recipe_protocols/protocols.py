...
class RatSSCxMainProtocol(ephys.protocols.Protocol):
    ...
    def generate_current(self, threshold_current=None, holding_current=None, dt=0.1):
        ...
        currents = {}
        curr_prefix = "current_"

        # rmp is step protocol
        time, current = self.rmp_protocol.generate_current(
            threshold_current=threshold_current, holding_current=holding_current, dt=dt
        )
        key = curr_prefix + self.rmp_protocol.name
        currents[key] = {"time": time, "current": current}

        for pre_protocol in self.pre_protocols:
            time, current = pre_protocol.generate_current(
                threshold_current=threshold_current,
                holding_current=holding_current,
                dt=dt,
            )
            key = curr_prefix + pre_protocol.name
            currents[key] = {"time": time, "current": current}

        for other_protocol in self.other_protocols:
            time, current = other_protocol.generate_current(
                threshold_current=threshold_current,
                holding_current=holding_current,
                dt=dt,
            )
            key = curr_prefix + other_protocol.name
            currents[key] = {"time": time, "current": current}
    ...
    def run(self, cell_model, param_values, sim=None, isolate=None, timeout=None):
        ...
        if not hasattr(cell_model, "threshold_current"):
            raise Exception(
                ...
                "that doesnt have threshold current value set: %s" % str(cell_model),
            )
