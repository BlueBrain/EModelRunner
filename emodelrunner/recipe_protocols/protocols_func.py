...
from emodelrunner.recipe_protocols.locations import (
    NrnSomaDistanceCompLocation,
    NrnSomaDistanceCompLocationApical,
)
from emodelrunner.recipe_protocols.features import define_efeatures
...
def get_recordings(protocol_name, protocol_definition, prefix, apical_point_isec):
    ...
    recordings.append(
        RecordingCustom(
            name="%s.%s.soma.%s" % (prefix, protocol_name, "v"),
            location=soma_loc,
            variable="v",
        )
    )
    ...
    if "extra_recordings" in protocol_definition:
        for recording_definition in protocol_definition["extra_recordings"]:
            ...
            recording = RecordingCustom(
                name="%s.%s.%s.%s" % (prefix, protocol_name, location.name, "v"),
                location=location,
                variable="v",
            )
...
def add_protocol(
    protocols_dict, protocol_name, protocol_definition, recordings, stochkv_det, prefix
):
...
def set_main_protocol_efeatures(protocols_dict, efeatures, morphname):
...
def create_protocols(
    ...
    with open(os.path.join(recipe_path), "r", encoding="utf-8") as f:
        recipes = json.load(f)
    recipe = recipes[etype]

    # a config folder with protocols, features and params files should be present
    ...
    morphval = morphs[0]
    if len(morphval) == 3:
        morphname, _, _ = morphval
    else:
        morphname, _ = morphval
    ...
    set_main_protocol_efeatures(protocols_dict, efeatures, morphname)
